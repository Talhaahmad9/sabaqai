...
export default function OnboardingShell() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Force full page reload so NextAuth re-reads session with updated onboarded flag
    ...
    <div className="w-full max-w-lg bg-slate-800 rounded-2xl shadow-2xl p-8 border border-slate-700">
      ...
      <div className="overflow-hidden min-h-72">
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            {step === 1 && (
              <StepProgram value={program} onChange={setProgram} />
            )}
            {step === 2 && (
              <StepSemester value={semester} onChange={setSemester} />
            )}
            {step === 3 && (
              <StepSubjects subjects={subjects} onChange={setSubjects} />
            )}
          </motion.div>
        </AnimatePresence>
      </div>
