...
export async function POST(request) {
  try {
    ...
    // Get user's uploaded materials for citation fallback
    const materials = await Material.find({ userId: user._id }).lean();
    const fileCitations = materials.map((m) => ({
      pageContent: "",
      location: m.fileName,
    }));

    // Use KB citations if available, otherwise show uploaded files
    const finalCitations =
      kbRes.citations?.length > 0 ? kbRes.citations : fileCitations;
    ...
    if (subject) {
      await User.findByIdAndUpdate(user._id, {
        $addToSet: { studiedTopics: subject },
      });
    }
    ...
    if (subject) {
      ...
      const occurrences = userMessages.reduce(
        (acc, msg) => acc + (msg.includes(subjectLower) ? 1 : 0),
        0,
      );
      if (occurrences > 2) {
        await User.findByIdAndUpdate(user._id, {
          $addToSet: { weakTopics: subject },
        });
      }
    }
