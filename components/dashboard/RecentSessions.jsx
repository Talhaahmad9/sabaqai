...
function SessionCard({ session }) {
  ...
  return (
    <Link
      href={`/chat/${session._id}?subject=${encodeURIComponent(session.subject ?? "General")}`}
      className="bg-slate-800 border border-slate-700 hover:border-violet-500 rounded-xl p-4 flex items-center gap-4 transition-colors group"
    >
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <ChevronRightIcon className="h-4 w-4 text-slate-600 group-hover:text-violet-400 transition-colors" />
      </div>
    </Link>
  );
}
