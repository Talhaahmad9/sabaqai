...
function SidebarContent({ collapsed, subjects, activeSubject, onSubjectSelect, onNewSubject }) {
  return (
    <div className="flex flex-col h-full">
      {/* Logo */}
      <div className={`flex items-center h-14 px-4 shrink-0 ${collapsed ? 'justify-center' : 'justify-start'}`}>
        {collapsed ? (
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden px-3 py-4 space-y-1">
        ...
        >
          ...
          {!collapsed && (
            <span className="text-sm font-medium whitespace-nowrap">New Subject</span>
          )}
        </button>
      </div>
      ...
      <div className={`px-3 pb-4 space-y-1 ${collapsed ? 'flex flex-col items-center' : ''}`}>
        <button
...
export default function Sidebar({ subjects, activeSubject, onSubjectSelect, onNewSubject, isMobileOpen, onMobileClose }) {
          ...
          <>
            ...
            >
              <SidebarContent
                ...
                subjects={subjects}
                activeSubject={activeSubject}
                onSubjectSelect={(id) => { onSubjectSelect(id); onMobileClose(); }}
                onNewSubject={() => { onNewSubject(); onMobileClose(); }}
              />
            </motion.aside>
          </>
        )}
